...
from dataloaders.la_heart import LAHeart, LAHeart_unseg, RandomCrop, CenterCrop, RandomRotFlip, ToTensor, TwoStreamBatchSampler
from dataloaders.la_heart import TransformConsistantOperator

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='F:/TsingHua-PearlRiverDelta-Phase1/CTM/CTM/CTM_data/Segmented')
parser.add_argument('--exp', type=str,  default='UAMT_unlabel', help='model_name')
parser.add_argument('--max_iterations', type=int,  default=300, help='maximum epoch number to train')
...
if __name__ == "__main__":
    ...
    db_train_labeled = LAHeart(base_dir=train_data_path,
                       split='train',
    ...
    db_train_unlabeled = LAHeart_unseg(base_dir=train_data_path,
                        split='train',
                        transform = transforms.Compose([
                          RandomRotFlip(),
                          RandomCrop(patch_size),
    ...
    # unlabeled_idxs = list(range(17, 64))
    ...
    labeled_trainloader = DataLoader(db_train_labeled, batch_size=labeled_bs, num_workers=0, pin_memory=True, worker_init_fn=worker_init_fn)
    unlabeled_trainloader = DataLoader(db_train_unlabeled, batch_size=batch_size-labeled_bs, num_workers=0, pin_memory=True, worker_init_fn=worker_init_fn)
    ...
    iter_num = 0
    max_epoch = max_iterations//len(unlabeled_trainloader)+1
    lr_ = base_lr
    model.train()
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, (sampled_batch_labeled, sampled_batch_unlabeled) in enumerate( zip(labeled_trainloader, unlabeled_trainloader) ):
            ...
            #volume_batch = torch.stack((labeled_volume_batch, unlabeled_volume_batch), 0)
            ...
            #preds = torch.zeros([stride * T, 2, 112, 112, 80])
            preds = torch.zeros([stride * T, num_classes, patch_size[0], patch_size[1], patch_size[2]])
            for i in range(T//2):
                TCO = TransformConsistantOperator(k=i, axis=np.random.randint(0, 2))
                ema_inputs = volume_batch_r + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2, 0.2)
                ema_inputs = torch.from_numpy( TCO.transform(ema_inputs) )
                with torch.no_grad():
                    pred = ema_model(ema_inputs)
                    pred = TCO.inv_transform(pred)
            ...
            if iter_num % 50 == 0:
                image = volume_batch[0, 0:1, :, :, 20:61:10].permute(3, 0, 1, 2).repeat(1, 3, 1, 1)#repeat 3是为了模拟图像的RGB三个通道
